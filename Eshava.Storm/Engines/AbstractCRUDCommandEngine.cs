...
namespace Eshava.Storm.Engines
{
	internal abstract class AbstractCRUDCommandEngine
	{
		...
		public virtual void ProcessInsertRequest<T>(CommandDefinition<T> commandDefinition) where T : class
		{
			...
			{
				AppendGeneratedPrimaryKeyQuery(sql, entityTypeResult);
			}
			else
			{
				sql.AppendLine(";");
		...
		/// <summary>
		/// Completes the insert statement so that it returns the generated key. The statement ends with its VALUES list.
		/// </summary>
		protected virtual void AppendGeneratedPrimaryKeyQuery(StringBuilder sql, Entity entity)
		{
			sql.AppendLine(";");
		...
		/// <summary>
		/// Quotes a column name in the current dialect
		/// </summary>
		protected static string QuoteIdentifier(string name)
		{
			return SqlDialects.Current.QuoteIdentifier(name);
		}
	}
}